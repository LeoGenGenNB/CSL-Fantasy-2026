...
const Pitch: React.FC<PitchProps> = ({ team, players, onPlayerClick, showPoints }) => {
  // Sort squad logic for rendering
  // Starters must be grouped by Position (GK -> DEF -> MID -> FWD)
  // Bench follows same order
  
  const starters = team.squad.filter(s => s.isStarter);
  const bench = team.squad.filter(s => !s.isStarter);

  // Helper to render a specific group of starters
  ...
  return (
    <div className="w-full max-w-2xl mx-auto select-none">
      ...
      <div className="mt-6 bg-slate-100 rounded-xl p-4 shadow-inner border border-slate-200">
        <h3 className="text-xs uppercase font-bold text-slate-500 mb-2 tracking-wider">Bench / 替补席</h3>
